  ...
  return (
    ...
    >
      ...
      <div className="relative mb-4">
        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5" style={{ color: 'hsl(var(--passport-gold-muted) / 0.4)' }} />
        <Input
          placeholder="Search people..."
          value={searchQuery}
          onChange={(e) => handleSearch(e.target.value)}
          className="pl-9 h-9 text-sm rounded-lg border-none"
          style={{ background: 'hsl(var(--passport-navy-light))', color: 'hsl(var(--passport-gold))' }}
        />
      </div>
      ...
      {searchResults.length > 0 && (
        <div className="mb-4 space-y-2">
          {searchResults.map((p) => (
            <div key={p.id} className="flex items-center justify-between px-3 py-2 rounded-lg" style={{ background: 'hsl(var(--passport-navy-light))' }}>
              <div className="flex items-center gap-2 min-w-0">
                <Avatar className="w-7 h-7">
                  <AvatarImage src={p.avatar_url || undefined} />
                  <AvatarFallback className="text-xs bg-muted">{getInitials(p.full_name)}</AvatarFallback>
                </Avatar>
                <span className="text-xs truncate" style={{ color: 'hsl(var(--passport-gold))' }}>{p.full_name || p.email}</span>
              </div>
              <Button variant="ghost" size="sm" onClick={() => handleAddFriend(p.id)} className="h-7 px-2">
                <UserPlus className="w-3.5 h-3.5" style={{ color: 'hsl(var(--passport-gold-muted))' }} />
              </Button>
            </div>
          ))}
        </div>
      ...
      {friends.length === 0 && !isLoading ? (
        <p className="text-sm text-center py-4" style={{ color: 'hsl(var(--passport-gold-muted) / 0.4)' }}>
          No friends yet
        </p>
      ) : (
        <ScrollArea className="w-full -mx-6 px-6">
          <div className="flex gap-3 pb-2">
            {friends.map((f, i) => (
              ...
              >
                <div className="w-12 h-16 rounded-md overflow-hidden border" style={{ borderColor: 'hsl(var(--passport-gold) / 0.2)', background: 'hsl(var(--passport-navy-light))' }}>
                  {f.profile.avatar_url ? (
                    <img src={f.profile.avatar_url} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-xs font-semibold" style={{ color: 'hsl(var(--passport-gold-muted))' }}>
                      {getInitials(f.profile.full_name)}
                    </div>
                  )}
                </div>
                <span className="text-[10px] truncate w-full text-center" style={{ color: 'hsl(var(--passport-gold-muted))' }}>
